...
export default function AIGlobalOptimizeModal({ isOpen, onClose, data, onApply }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-3xl flex flex-col max-h-[90vh] border border-gray-200 dark:border-gray-800">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center bg-indigo-50 dark:bg-indigo-950/40 rounded-t-xl">
        ...
        <div className="p-6 flex-1 overflow-y-auto space-y-4">
           ...
           <div className="border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-lg p-6 flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-950 hover:bg-gray-100 dark:hover:bg-gray-800/50 transition">
              <Upload size={24} className="text-gray-400 dark:text-gray-500 mb-2" />
              <label className="cursor-pointer text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-medium text-sm">
                {isFrench ? 'Cliquez pour uploader un fichier texte (.txt)' : 'Click to upload a text file (.txt)'}
                <input type="file" accept=".txt" className="hidden" onChange={handleFileUpload} />
              </label>
           </div>

           <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400 my-2 w-full">
        ...
        <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-800 flex justify-end gap-3 bg-gray-50 dark:bg-gray-950 rounded-b-xl">
          <button onClick={onClose} disabled={loading} className="px-4 py-2 border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded bg-white dark:bg-gray-900 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
            {isFrench ? 'Annuler' : 'Cancel'}
          </button>
          <button 
            onClick={handleOptimize} 
            disabled={!detailedDoc.trim() || loading}
            className="flex items-center gap-2 px-5 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 dark:disabled:bg-indigo-900/40 disabled:text-gray-400 text-white font-medium rounded transition"
          >
            {loading ? (
